...
{
    public partial class AddInsuranceCoupon : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                int MasterIds = (int)SiteKey.MasterParent.InsuranceCoupon;
        ...
        private void FillTable()
        {
            try
            {
                ...
                var Tot = Global.Context.InsuranceIndividuals.SingleOrDefault(p => p.InsuranceIndividualId == Id);

                try
                {
                    ExpiryDateofPolicy.Text = Tot.PolicyExpiryDate.Value.ToString("dd/MM/yyyy");
                }
                catch { }
                VehicleNo.Text = Tot.VehicleRegnNo.ToString();
                CustomerName.Text = Tot.CustomerName.ToString();
                MIAdvisorName.Text = Tot.DealersExecutive;
                var Payment = Global.Context.InsuranceCoupons.AsEnumerable().Where(p => p.InsuranceIndividualId == Id).ToList();
        ...
        {
            var obj = Global.Context.InsuranceCoupons.SingleOrDefault(p => p.InsuranceCouponId == v);
            {
        ...
        protected void savelocation_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (Page.IsValid)
                {
                    ...
                    var Tot = Global.Context.InsuranceIndividuals.SingleOrDefault(p => p.InsuranceIndividualId == Id);
                    if (Tot.PolicyExpiryDate.Value.Date >= ExpiryDateofPolicy.Text.DateConvertText().Date)
                    {
                        ...
                        if (LIBS.Common.ConvertInt(savelocation.CommandArgument) > 0)
                        {
                            using (TransactionScope Trans = new TransactionScope())
                            {
                                ...
                                var obj = Global.Context.InsuranceCoupons.SingleOrDefault(p => p.InsuranceCouponId == projectid);
                                {
                            ...
                            Response.Redirect("AddInsuranceCoupon.aspx?InsuranceIndividualId=" + Id + "", false);
                        }
                        else
                        {
                            using (TransactionScope Trans = new TransactionScope())
                            {
                                Business.InsuranceCoupon obj = new Business.InsuranceCoupon();


                                //var objcode = Global.Context.ExecuteStoreQuery<GEtNo>("select   ((select top 1 Code from [Master] where Master.MasterId=" + MasterId.SelectedValue + ")+'/20-21/'+ REPLACE(STR(convert (nvarchar(50),((select COUNT(*)+1 from InsuranceCoupon where Comp_Code=" + SiteSession.Comp_MstSession.Comp_Code+" and MasterId="+MasterId.SelectedValue+"))),5),' ', '0')) as code").FirstOrDefault().code;
...

                    {
                        ...
                        MessageBox.ShowMessage("Error", " Expiry Date Not  allow", SiteKey.MessageType.danger);
                    }
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (STREmailId.Length > 0)
                {
                    ...
                    objListOfDocument.Add(new Emailer.AttachmentStream() { FileName = "Coupon", File = Common.CreatePDF(gtt, SiteSession.Comp_MstSession.Comp_Name), Extranson = "pdf", subExtranson = "pdf" });
        ...
        protected void PrintData_Click(object sender, EventArgs e)
        {
            ...
            string url = "print/multipalcouponprint.aspx?Id=" + gtt + "&&CompName=" + SiteSession.Comp_MstSession.Comp_Name + "";
